import sqlite3
conexao = sqlite3.connect('phoenixrise.db')
...
CREATE TABLE crossfits (
    id_crossfit INTEGER PRIMARY KEY AUTOINCREMENT,
    nome TEXT,
    mensalidade REAL,
    descricao TEXT,
    latitude REAL,
...
CREATE TABLE usuarios (
    ...
    foto_url TEXT,
    nivel_esportivo TEXT,
    interesses TEXT
);

-- ================================
-- TABELA: USUARIO_ESPORTE
-- ================================
CREATE TABLE usuario_esporte (
    ...
    FOREIGN KEY (id_esporte) REFERENCES esportes(id_esporte) ON DELETE CASCADE ON UPDATE CASCADE
);
...
('Academia Prime Core', 85.00, 'A Prime Core oferece treinos funcionais e musculação de alta performance em ambiente moderno.', -5.650346879472210, -37.79995285916965, 'Apodi', 'R. Joaquim de Moura', 'Ao lado da Facis', 'Seg-Sex(3:50h-21h) Sab-Dom(15h-19h)', 'primecoreofc', 'https://res.cloudinary.com/drdezhuko/image/upload/v1763074766/kxurlorbnhtuofxvdyhy.png'),
...
('Academia Performance', 85.00, 'A Academia Performance é ideal para quem busca força e resistência com acompanhamento profissional.', -5.665555680785641, -37.79491094724948, 'Apodi', 'Av. Moésio Holanda', 'Em frente à Escola Estadual', 'Seg-Sex(5h-21h) Sab(6h-12h)', 'performanceapodi', 'https://res.cloudinary.com/drdezhuko/image/upload/v1763075435/ob7oojqgcqgcvmxtfhbj.png');

-- ================================
-- TABELA: ACADEMIAS LIVRES
-- ================================
CREATE TABLE academias_livres (
    id_academia_livre INTEGER PRIMARY KEY AUTOINCREMENT,
    nome TEXT NOT NULL,
    latitude REAL,
    longitude REAL,
    cidade TEXT,
